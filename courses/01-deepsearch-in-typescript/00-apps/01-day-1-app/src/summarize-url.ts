...
Please synthesize the content above, focusing on information relevant to the research topic and search query. Remember to maintain the original context and never make up information.`,
    }),
    {
      maxRetries: 2,
      initialDelay: 2000,
      shouldRetry: (error) => {
        // Retry on 503 (overload) or other retryable errors
        return error.statusCode === 503 || error.isRetryable === true;
      }
    }
  );
