...
export const checkAnswerRelevancy = async (opts: {
  ...
  if (statements.length === 0) {
    return {
      ...
      metadata: {
        statements: [],
        verdicts: [],
        rationale: "Empty submission",
      },
    };
  }

  // Step 2: Evaluate each statement for relevancy
  ...
  return {
    ...
    metadata: {
      ...
      rationale: `${verdictsResult.verdicts.filter(v => v.verdict === "yes").length} relevant, ${
        verdictsResult.verdicts.filter(v => v.verdict === "unsure").length
      } partially relevant, ${
        verdictsResult.verdicts.filter(v => v.verdict === "no").length
      } not relevant statements`,
    },
  };
};
...
>({
  ...
  description: "Evaluates how relevant the answer is to the question by breaking it down into statements",
  scorer: async ({ input, output }) => {
    // Extract the question from the Message array
    const messages = input as any[];
    const question = messages?.[0]?.content || input;
